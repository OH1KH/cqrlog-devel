...
type
  ...
  private
    ...
    FMyLoc: String;
    NowDate : String;
    FFilteredOutRecNr: integer;        FFilterDateRange: array [0..1] of TDateString;
  ...
  public
    ...
    function storeSingleAdifStringToCqrlogMain(var adifString:string;var checkForDuplicates:Boolean) :boolean;
    { public declarations }
  end; 
...
function TfrmAdifImport.storeSingleAdifStringToCqrlogMain(var adifString:string;var checkForDuplicates:Boolean):boolean;
// This function should store a single ADIF formatted string in cqrlog_main.
// it will return false if it could not save to database
// optional you can avoid to save doubles

// Todo DL7OAP:
//              - implement DuplicateCheck
//              - check if this function works (was implemented blind without testing)
//
var
  ...
  ErrText : String = '';
begin
  storeSingleAdifStringToCqrlogMain:=False;
  initializeTypeVariable(d);
  FFilterByDate:=False;
  adifString:=Upcase(adifString);
  while getNextAdifTag(adifString,adifTag,data) do
  begin
    ...
    begin
      if not saveNewEntryFromADIFinDatabase(d,ErrText) then
        ...
        storeSingleAdifStringToCqrlogMain:=True
    end;
